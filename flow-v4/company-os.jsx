...
const V4_COMPANY_OS_PREP = [
  {
    title: 'AZ8 / invoice is out, prepaid policy still needs hard reset',
    tags: ['P0', 'invoice sent', 'payment first'],
    points: [
      'Kevin asked for Payoneer payment details and Asher already sent invoice_AZ8_063026.pdf for the $1,200 Zumi quote-post lane.',
      'The problem is unchanged: Kevin still wants payment processed within two business days after the post goes live, which does not clear execution.',
      'Asher owns the reply that resets prepaid terms, asks for wire or Payoneer receipt, and requests the final source post plus brief only after payment is real.',
      'This is not Robert-ready until the receipt lands in-thread.',
    ],
  },
  {
    title: 'Viture / July sync and giveaway details are in, payment proof is still missing',
    tags: ['P0', 'schedule confirm', 'receipt missing'],
    points: [
      'Leo replied on July 2 with the Beast giveaway mechanics, #VITURE tag requirement, week-of-July-13 launch target, and a request to lock the first strategy sync for Monday, July 6, 2026 at 9:00 AM PT.',
      'Viture will handle winner shipping, but the email thread still does not show the actual payment receipt for Month 1.',
      'Asher owns the calendar confirmation, the receipt chase, and the 60-second Robert brief for Beast plus giveaway structure once finance clears.',
      'Useful prep is now in hand, but finance is still not done.',
    ],
  },
  {
    title: 'ACL / outlines are approved, execution stays inside the July 5 box',
    tags: ['P0', 'approved outlines', 'scope hold'],
    points: [
      'Annika confirmed on July 2 that the new outline passed Alibaba review and asked the team to continue content preparation.',
      'The active work is now Robert prep, Qoder/Qwen talking points, and keeping every logistics, reimbursement, and client detail in the same email chain.',
      'Asher owns the prep brief and any final schedule cleanup, but should not reopen July 6 or July 7 unless the commercial terms change first.',
      'Approved outlines mean prep should continue, not that scope gets looser.',
    ],
  },
  {
    title: 'AD&OG / screenshot landed, finance verification and reschedule still open',
    tags: ['P0', 'payment screenshot', 'date reset'],
    points: [
      'Evie sent a payment screenshot on June 30 after accepting the 100 percent upfront rule for the $1,995 custom X post, and Asher already asked for a new date because the June 24 timing is gone.',
      'The lane is now finance confirmation plus calendar reset, not fresh selling.',
      'Asher owns proof verification, the new scheduled window, and the Robert brief once the payment trail is confirmed beyond the screenshot.',
      'A screenshot alone is still provisional.',
    ],
  },
  {
    title: 'iLands / new lead needs scoped reply and price lock',
    tags: ['P1', 'new lead', 'asher reply'],
    points: [
      'A new inbound on July 2 pitched a paid X quote-post for iLands around AI agents with fixed compute budgets and explicit openness to Robert\'s standard rate.',
      'There is no scope conflict yet, no payment friction yet, and no brief debt yet. This is still a clean sales reply moment.',
      'Asher owns the first response with recommended format, rate, and timing options before the thread cools off.',
      'Fresh leads should get a clean commercial answer before they become stale watch items.',
    ],
  },
];
...
const V4_COMPANY_OS_WAITING = [
  {
    title: 'OJO / product access is real again, but the deal just moved back into pricing',
    tags: ['watch', 'budget pushback', 'brief on hold'],
    points: [
      'EezyCollab reopened OJO with the live brief, invitation code, and a same-day request for a script, then immediately pushed back on pricing after Asher reset the rates.',
      'The current thread has product access and brief material, but not an agreed commercial package.',
      'Keep it warm and ready for a fast restart, but do not spend Robert time on script work while the price is unsettled.',
      'Pricing disagreement outranks fresh brief enthusiasm.',
    ],
  },
  {
    title: 'Acti / live launch is done; future work waits for the next milestone',
    tags: ['watch', 'do not chase', 'package resend only if needed'],
    points: [
      'AK said on July 2 that the current packages file appeared corrupted, but also made clear that Acti wants to revisit collaborations only around future campaign milestones.',
      'The June 30 launch post is already live, so this is not an active execution emergency.',
      'Do not proactively push here. Re-send pricing cleanly only when the next milestone actually reopens.',
      'This is a waiting lane, not a same-day follow-up lane.',
    ],
  },
  {
    title: 'OMANE / Apify payment is claimed sent, but proof and next Viktor plan are still missing',
    tags: ['watch', 'receipt missing', 'next-campaign hold'],
    points: [
      'Ori said the $1,000 Apify payment was sent and later said the team is still waiting on Viktor\'s final plan before discussing more monthly work.',
      'That leaves one admin gap and one commercial gap: receipt proof is not in-thread, and the next scope is not ready.',
      'Keep it warm, but the only justified poke is payment proof or a concrete next brief.',
      'A claimed payment plus vague future work is still a waiting state.',
    ],
  },
  {
    title: 'FORKOFF / WhatsApp moved faster than email, so the board cannot treat it as settled',
    tags: ['watch', 'off-thread', 'canonical facts missing'],
    points: [
      'JK asked to lock a schedule and payment process after shifting details to WhatsApp, and Asher replied that there was a misunderstanding and pushed the discussion back off-thread.',
      'The email record still does not show the exact deliverable, timing, or payment mechanics in a canonical way.',
      'Keep this waiting until the non-negotiables get restated cleanly in email or another durable source.',
      'Side-channel urgency is not the same thing as board clarity.',
    ],
  },
];
...
const V4_COMPANY_OS_RULES = [
  ...
  'If an attachment arrives corrupted, resend it once cleanly when asked, then stop chasing until the client reopens the milestone.',
