...
const V4_COMPANY_OS_PREP = [
  {
    title: 'Viture / scheduling moved again, and payment still needs a real receipt trail',
    tags: ['P0', 'schedule reply', 'receipt verify'],
    points: [
      'Leo replied on July 4 that Saturday at 10:00 AM PT does not work for the China side and asked for an evening PT slot after 5:00 PM instead.',
      'The Beast giveaway mechanics are already clear, so the remaining Asher move is a clean schedule reply plus a direct check on whether the sent payment actually landed.',
      'Do the 60-second Robert brief only after the sync time and finance proof sit in the same thread.',
      'Scheduling flexibility is useful, but the lane is still prep until the receipt trail is real.',
    ],
  },
  {
    title: 'ACL / Qoder answers and onsite contact are in, so prep should tighten instead of drift',
    tags: ['P0', 'brief prep', 'acl ready'],
    points: [
      'Annika sent Alibaba\'s written Qoder answers on July 4 and added the updated onsite contact, Nicki, as the primary ACL point person.',
      'That means the lane now has enough material to prep Robert cleanly even if the online interview timing is still unresolved.',
      'Asher owns the 60-second conference brief, the source-doc handoff, and keeping every contact/detail update inside the active email chain.',
      'Written answers help execution, but they do not justify new scope or date drift.',
    ],
  },
  {
    title: 'OJO + team9 / product notes are back, and the admin blockers should clear same day',
    tags: ['P0', 'reply now', 'payment admin'],
    points: [
      'EezyCollab sent the OJO script, extra visual/shareability notes, and a direct request for the account series number so they can top up the profile.',
      'They also said the $995 team9 payment will process as soon as the submitted form is accepted, so the path is operational, not speculative.',
      'Asher owns one clean reply that sends the series number, closes the form/proof loop, and pulls payment plus timing into the same thread.',
      'Do not build Robert\'s final brief until the approval path and money trail are explicit.',
    ],
  },
  {
    title: 'Humanoid / payment policy needs a final yes before invoice and draft timing can move',
    tags: ['P1', 'invoice next', 'policy hold'],
    points: [
      'Iván accepted the $1,995 Custom X Post plus the $5 Stripe bank transfer fee, then asked for a 50/50 structure tied to approval and go-live.',
      'Asher already gave the right counter: draft first, then full payment before publish, with the goal of posting next Tuesday or Wednesday.',
      'The next move is not more pitch work. It is a client confirmation that the prepay structure works so the invoice or payment link can go out immediately.',
      'No split-payment drift. No invoice without a clear yes.',
    ],
  },
  {
    title: 'Perceptron / two July launches are on deck, so payment shape has to settle first',
    ...
    points: [
      ...
      'Asher correctly pushed payment to the front and asked whether they want to settle one QRT or both together before prep starts.',
      'This is commercially closeable right now, and it should close before Robert burns time on early execution notes.',
      'If payment is unresolved, treat both launch dates as provisional.',
    ],
  },
  {
    title: 'Sintra / payment is done, but the missing video preview still blocks a clean finish',
    tags: ['P1', 'final asset', 'live link'],
    points: [
      'Julius said payment is already complete and asked for the missing video preview, plus the final tracking link that should be used in the video.',
      'This is no longer a sales lane. It is post-sale execution hygiene: send the preview or final post proof back into the same thread.',
      'Asher owns the asset handoff and the final link trail so the lane can move from “paid” to “fully evidenced.”',
      'Paid but missing the live asset is still an action lane.',
    ],
  },
];
...
const V4_COMPANY_OS_WAITING = [
  {
    title: 'Riverside / terms disagreement is active, so the next step is document retrieval, not heat',
    tags: ['watch', 'scope mismatch', 'agreement needed'],
    points: [
      'Savion says Riverside expected a card-link payment flow on net terms and is ready to share the agreement or prior email chain, while Asher says those terms were never relayed on the Unaligned side.',
      'Do not turn this into a memory fight. Wait for the agreement trail, then reconcile it against what was actually delivered.',
      'Until the documents are back in-thread, this is a watch lane rather than execution.',
      'Conflicting recollections lose to written terms.',
    ],
  },
  {
    title: 'OMANE / Apify claims are still softer than proof, so keep it visible but not loud',
    tags: ['watch', 'receipt missing', 'do not force'],
    points: [
      'Ori said the $1,000 Apify payment was sent and then immediately shifted to waiting on Viktor\'s final plan before discussing the next campaign.',
      'That leaves the same two gaps as before: no clean finance proof and no concrete next-campaign scope.',
      'Watch it, but do not manufacture urgency beyond a promised receipt or a real Viktor brief.',
      'Claimed sent is not settled.',
    ],
  },
  {
    title: 'xRoboHub / interview lane is waiting on client-side dates, so do not overwork it',
    tags: ['watch', 'schedule inbound', 'no chase yet'],
    points: [
      'May was told on July 3 that Robert is available in mid to late July for the one-hour interview and that specific dates or a calendar link are needed from their side.',
      'There is nothing useful to prep until that availability comes back from the client side.',
      'Keep it visible, but this is waiting on inbound scheduling rather than an Asher follow-up gap.',
      'A pending calendar link is not a blocker worth repeated pokes.',
    ],
  },
  {
    title: 'Stripe verification / payouts are paused until Sam clears identity review',
