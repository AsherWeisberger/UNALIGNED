// Pages-hosted guard: custom domain → public forms only; github.io → Mac ops host.
const V4_EARLY_PAGES_HOSTS = new Set(['asherweisberger.github.io', 'agentdashboard.cloud', 'www.agentdashboard.cloud']);
const V4_EARLY_CUSTOM_DOMAIN_HOSTS = new Set(['agentdashboard.cloud', 'www.agentdashboard.cloud']);
...
function V4EarlyShouldRedirectToConnect() {
  try {
    const host = String(window.location?.hostname || '').toLowerCase();
    if (!V4_EARLY_CUSTOM_DOMAIN_HOSTS.has(host)) return false;
...
function V4EarlyShouldRedirectToMac() {
  try {
    const host = String(window.location?.hostname || '').toLowerCase();
    if (host !== 'asherweisberger.github.io') return false;
...
function V4EarlyRedirectAtBoot() {
  if (V4EarlyShouldRedirectToConnect()) {
    window.location.replace('/connect/' + window.location.search + window.location.hash);
    return true;
  }
  if (V4EarlyShouldRedirectToMac()) {
...
const V4_COMPANY_OS_PREP = [
  {
    title: 'OJO / script feedback is back, but Asher still owns the activation handoff',
    tags: ['P0', 'reply now', 'brief prep'],
    points: [
      'EezyCollab came back on July 3 with a tentatively approved OJO script, extra brand notes on visual quality and shareability, and a direct ask for the account series number so they can top up the profile.',
      'The lane is now operational rather than speculative: the product access, brief, and draft direction are real, but the thread still needs one clean Asher reply that sends the series number, locks the case rationale, and pulls payment plus timing into the same chain.',
      'Do the 60-second Robert brief only after the case, approval path, and launch sequence are explicit.',
      'This is an action lane because the next move is a concrete Asher reply, not more waiting.',
    ],
  },
  {
    title: 'AZ8 / wire path is open, but the contract sheet and receipt are still missing',
    tags: ['P0', 'payment path', 'contract blocker'],
    points: [
      'Kevin now has a wire workflow in motion for the $1,200 Zumi quote repost, but his bank requires a signed contract sheet and SWIFT-ready payment details before the transfer can clear.',
      'He also said the Zumi video is scheduled to go live today, so the thread is trying to accelerate toward a repost immediately after payment lands.',
      'Asher owns the contract review decision, the receipt chase, and the hold line that keeps Robert off the calendar until the money is actually confirmed.',
      'An invoice and a spreadsheet are not payment proof.',
    ],
  },
  {
    title: 'Viture / sync details are ready, but finance proof still has not landed',
    tags: ['P0', 'meeting confirm', 'receipt missing'],
    points: [
      'Leo confirmed the Beast giveaway mechanics, #VITURE requirement, shipping ownership, and asked for the first strategy sync on Monday, July 6, 2026 at 9:00 AM PT.',
      'The July operating plan is real now, but the thread still only says payment was initiated. The actual receipt is still missing.',
      'Asher owns the calendar confirmation, the receipt follow-up, and the short Robert brief for Beast plus giveaway structure once finance clears.',
      'This is ready for prep, not ready for execution.',
    ],
  },
  {
    title: 'ACL / outlines are approved, so prep should stay tightly inside July 5',
    tags: ['P0', 'approved outlines', 'robert brief'],
    points: [
      'Annika confirmed on July 2 that Alibaba approved the revised eight-outline package and asked the team to continue content preparation.',
      'That means the work has shifted from negotiation into execution discipline: Robert prep, Qoder/Qoder Work talking points, and zero scope drift outside the agreed July 5 box.',
      'Asher owns the briefing and thread hygiene, not a new date hunt.',
      'Approved outlines are a green light for prep, not permission to reopen July 6 or July 7.',
    ],
  },
  {
    title: 'AD&OG / claimed payment exists, but finance confirmation and date lock do not',
    tags: ['P0', 'receipt verify', 'schedule reset'],
    points: [
      'Evie says payment was done and sent a screenshot, but the thread still has no clean confirmation that finance actually received the full $1,995 wire.',
      'At the same time, AD&OG is still asking for Robert\'s angle on Inplay while saying the posting date is being confirmed with the client.',
      'Asher owns proof verification, the rescheduled date, and the Robert idea handoff only after the payment trail is real.',
      'A screenshot plus “we are confirming the date” is still a prep state.',
    ],
  },
  {
    title: 'Perceptron / two July launches are coming, so payment shape has to get settled now',
    tags: ['P1', 'payment choice', 'launch prep'],
    points: [
      'Eric said Perceptron wants one QRT for the July 13 launch and another roughly seven days later, then asked when prep should start.',
      'Asher already pushed back with the right next step: settle payment first, then decide whether they are paying one by one or together.',
      'This lane now needs a fast commercial close so Robert prep can happen before the first launch window compresses.',
      'If payment is unresolved, do not treat the launch dates as real execution dates.',
    ],
  },
  {
    title: 'team9 / payment form is in, which makes this a quick admin unblock',
    tags: ['P1', 'payment admin', 'easy win'],
    points: [
      'EezyCollab sent a fresh July 3 form for the $995 team9 collaboration and said payment will process as soon as it is submitted.',
      'This is not strategy work. It is simple admin that should get cleared before it turns into another unnecessary waiting lane.',
      'Asher owns the form completion and proof trail back to the thread.',
      'Treat low-friction payment admin as same-day cleanup.',
    ],
  },
];
...
const V4_COMPANY_OS_WAITING = [
  {
    title: 'OMANE / Apify payment is claimed sent, but proof and Viktor scope are still absent',
    tags: ['watch', 'receipt missing', 'next-campaign hold'],
    points: [
      'Ori said the $1,000 Apify payment was sent, then immediately shifted the conversation to waiting on Viktor\'s final plan before talking about the next campaign.',
      'That leaves two open gaps and neither is solved in-thread: finance still is not confirmed, and the next scope is still vague.',
      'Watch it, but do not manufacture urgency beyond receipt proof or a concrete Viktor brief.',
      'Claimed sent is not the same as cleared.',
    ],
  },
  {
    title: 'Acti / the launch is done and the next move belongs to their milestone timing',
    tags: ['watch', 'do not chase', 'package resend only if needed'],
    points: [
      'AK said the package file seemed corrupted, but also made clear that Acti wants to revisit paid work only around future campaign milestones.',
      'Asher already resent the pricing cleanly. Nothing else is justified until they reopen the next milestone.',
      'This is a do-not-chase lane unless the client explicitly comes back in.',
      'A corrupted attachment is an admin fix, not a reason to restart sales pressure.',
    ],
  },
  {
    title: 'Riverside / terms disagreement is active, so this needs documents before emotion',
    tags: ['watch', 'scope mismatch', 'agreement needed'],
    points: [
      'Savion says Riverside expected payment by card link on net terms and is ready to share the agreement or prior thread, while Asher says that scope was never relayed on the Unaligned side.',
      'Do not let this become a feelings argument. Wait for the agreement trail, then reconcile terms against what actually got delivered.',
      'Until the documents are back in-thread, this is a watch item rather than a fresh execution lane.',
      'Conflicting memories lose to written terms.',
    ],
  },
  {
    title: 'xRoboHub / Influcio interview lane is waiting on dates from their side',
    tags: ['watch', 'schedule inbound', 'no chase yet'],
    points: [
      'May was told on July 3 that Robert is available in mid to late July for the one-hour interview and needs a calendar link or specific dates to lock it in.',
      'There is nothing to prep until the scheduling options arrive from their client side.',
      'Keep it visible, but this is waiting on inbound availability rather than an Asher follow-up gap.',
      'A pending calendar link is not a blocker worth repeated pokes.',
    ],
  },
];
...
const V4_COMPANY_OS_RULES = [
  ...
  'A screenshot, wire promise, or contract sheet is still provisional. Treat it as prep until the cash trail matches.',
  ...
  'Re-send broken files once, then stop pushing until the client actually reopens the milestone.',
  'Do-not-touch means no proactive poke unless there is a promised receipt, promised live link, or promised scheduling option that should already be in hand.',
...
function V4LoadGodModeModule() {
  ...
  window.__godModeModulePromise = new Promise((resolve, reject) => {
    ...
    s.src = 'flow-v4/god-mode-earth.js?v=20260704-godmode-v14';
    s.async = true;
    s.onload = () => {
