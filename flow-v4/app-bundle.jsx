...
const V4_COMPANY_OS_PREP = [
  {
    title: 'ACL / second payment invoice is out, and Robert now needs one tight field brief',
    tags: ['P0', 'payment chase', 'brief prep'],
    points: [
      'Asher sent the July 6 final-payment invoice for $12,300 on July 4, and Annika also delivered the written Qoder feedback plus Nicki as the onsite ACL contact.',
      'That gives Asher enough material to prep Robert now, but not enough freedom to wing it: Qoder Work cannot become a standalone interview, frontal face footage is restricted, and Alibaba wants every video approved before publish.',
      'The Asher lane is receipt chase plus one 60-second Robert execution brief that locks product boundaries, interview constraints, contact names, and the July 5 field order into one pass.',
      'Written prep is ready. Payment proof for the second installment is not.',
    ],
  },
  {
    title: 'Viture / pick the sync time, then force payment proof into the same thread',
    tags: ['P0', 'schedule reply', 'receipt verify'],
    points: [
      'Leo said on July 4 that Saturday at 10:00 AM PT does not work for the China side and asked for an evening PT slot after 5:00 PM, after first floating July 8 or July 9 at 9:00 AM PT.',
      'The Beast giveaway mechanics are already clear, so this is no longer a strategy problem. It is a schedule-lock plus remittance-proof problem.',
      'Asher should reply with one workable slot and ask for the actual payment receipt in the same note so the meeting and money trail stop drifting apart.',
      'Do not build Robert\'s brief until the sync time and finance proof sit together in-thread.',
    ],
  },
  {
    title: 'AD&OG / launch-day source pack is still incomplete even if payment was claimed',
    tags: ['P0', 'payment proof', 'source link'],
    points: [
      'Evie said the InPlay content should publish when the app hits the App Store around July 6, with the official promo video and exact posting schedule still to come.',
      'The client also wants Robert\'s adapted caption back for approval, but Asher is still waiting on a fully reliable payment trail and the canonical source asset.',
      'Asher owns the receipt check, the official video/source-link chase, and the exact posting-time confirmation before this touches Robert again.',
      'No launch-day post should run from placeholder copy alone.',
    ],
  },
  {
    title: 'AZ8 / the wire is blocked on contract language, so clear the admin path or stop pretending it is paid',
    tags: ['P1', 'contract ask', 'invoice hold'],
    points: [
      'Kevin said the Chinese bank needs a signed contract before it can process the international wire, then asked whether the quote repost can move as soon as payment arrives.',
      'Asher already sent the invoice PDF, but the thread still does not show a settled answer on whether invoice-only works or whether a signable contract or extra fields need to be provided.',
      'This is an Asher admin lane now: close the contract path, keep the invoice trail clean, and ask for the receipt the moment the wire goes out.',
      'No payment proof means no scheduling promise to Robert.',
    ],
  },
  {
    title: 'Riverside / the signed agreement is finally in-thread, so the dispute has to become factual',
    tags: ['P1', 'agreement review', 'deliverable check'],
    points: [
      'Jay told Asher on July 4 that they will pay out of pocket while waiting on Riverside, and the July 5 thread now includes the signed Riverside agreement PDF.',
      'Savion still says payment is net 30 from the posting date and that the Facebook cross-post plus analytics package are still part of the owed deliverables.',
      'Asher has to stop arguing from memory, pull the written terms, and decide whether the missing Facebook link is a real deliverable gap or a scope mismatch that should be pushed back with documents.',
      'This is actionable now because the paperwork finally exists in-thread.',
    ],
  },
];
...
const V4_COMPANY_OS_WAITING = [
  {
    title: 'OMANE / Apify still says payment sent, but the useful next step is Viktor, not another poke',
    tags: ['watch', 'receipt missing', 'do not force'],
    points: [
      'Ori said the $1,000 Apify payment was sent, and the thread then shifted immediately to waiting on Viktor\'s final plan for the next monthly campaign.',
      'That leaves the same real gap as before: no receipt trail yet, but also no current execution request that justifies noise.',
      'Keep it visible until proof lands or Viktor comes back with actual scope.',
      'Claimed sent is still not settled.',
    ],
  },
  {
    title: 'xRoboHub / the ball is on their side for interview dates, so do not manufacture motion',
