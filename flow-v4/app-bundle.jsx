...
const V4_COMPANY_OS_PREP = [
  {
    title: 'ACL / July 5 scope still overloaded',
    tags: ['P0', 'outlines due', 'schedule drift'],
    points: [
      'Annika now wants the 8 outlines tomorrow, says the Qoder lead cannot do July 5, and asked to move that session to July 6 or 7 even though the Qwen Cloud interview assets and answers are already back in hand.',
      'Unaligned already held the line that July 5 stays the only covered on-site day: paper authors at 1:00-1:30 PM PDT, Qoder Work at 1:30-2:00 PM PDT, and Qoder must either fit July 5 or fall back to written Q&A.',
      'Asher owns the 8-outline batch, the 60-second Robert prep from the Qwen Cloud recording and answer doc, and the ticket / reimbursement cleanup trail.',
      'Scope drift after sign-off is not automatic approval. Keep every ACL logistics change inside the active email chain and do not casually open July 6 or 7.',
    ],
  },
  {
    title: 'Riverside / green light after 9 AM PT',
    tags: ['P0', 'go window', 'brief now'],
    points: [
      'Savion sent the Riverside 2.0 PR brief on June 30 and confirmed the post is cleared to run any time after 9:00 AM PT.',
      'This lane is no longer blocked on launch date. It is now blocked on Robert-ready copy, tags, and a clean approval path pulled from the brief.',
      'Asher owns the 60-second Robert brief, final caption handoff, and any same-thread launch confirmation back to Savion and Jay.',
      'A press brief is prep material, not the final live artifact. Do not call Riverside closed until the real post URL is back in the thread.',
    ],
  },
  {
    title: 'AD&OG / payment proof sent, brief next',
    tags: ['P0', 'payment sent', 'verify first'],
    points: [
      'Evie replied on June 30 saying the AD&OG payment is done and attached a screenshot after Asher held the 100 percent upfront policy.',
      'The next step is not more negotiation. It is finance verification, then a tight Robert angle / asset ask so the custom X post can move.',
      'Asher owns payment verification, the Robert prep note, and the request for any must-use assets or talking points once funds are confirmed.',
      'A screenshot receipt is not settled money until the finance trail matches it.',
    ],
  },
  {
    title: 'Cline / accepted at $2,195, still no invoice trail',
    tags: ['P0', 'invoice now', 'timing slipped'],
    points: [
      'Pratham accepted the revised $2,195 quote-post package with engagement and asked for June 29 at 8:45 AM PT, but the thread still only shows billing details and the creator brief.',
      'There is still no visible sent invoice, payment proof, or canonical source post for Robert to quote from, and the requested launch window has already slipped.',
      'Asher owns the invoice send, payment capture, and the revised timing / source-link reset before Robert touches execution.',
      'A yes plus billing details is not a booked collab. The invoice trail, paid status, and live source link still have to exist in the same thread.',
    ],
  },
  {
    title: 'OMANE / Viktor closeout still open',
    tags: ['P1', 'closeout due', 'live links'],
    points: [
      'Ori said the second Viktor leg is settled on their end, then pushed for the remaining LinkedIn post and X thread to be live before July.',
      'Asher replied that both would be up tomorrow, which means the open problem is now execution closeout more than invoice chasing.',
      'Asher owns the final Robert live links plus the Fillout logging so the thread closes cleanly.',
      'Settled payment is not a closed campaign until both promised live URLs are back in-thread.',
    ],
  },
  {
    title: 'Sintra / retest after credits restore',
    tags: ['P1', 'lead reply', 'product retest'],
    points: [
      'Julius resurfaced on June 29, apologized for the silence, and added 100 credits to AsherUnaligned@gmail.com after the earlier credit and connector failures.',
      'That is enough to reopen testing, but not enough to call the lane clean. Someone still has to prove the helper flow works end-to-end on the restored account.',
      'Asher owns the clean retest and the exact failure notes if the connector / credit issue still breaks the flow.',
      'Restored credits are not proof that the campaign is execution-ready.',
    ],
  },
];
...
const V4_COMPANY_OS_WAITING = [
  {
    title: 'Viture / finance initiated, receipt still missing',
    tags: ['watch', 'payment initiated', 'kickoff inputs'],
    points: [
      'Leo confirmed Viture started the payment process and still wants July 1 as the month-one kickoff target.',
      'What is still missing is the actual receipt plus the first-phase priorities, dates, and roadmap inputs.',
      'Keep this warm, but do not allocate Robert time until the receipt and kickoff pack are actually in-thread.',
      'Finance is processing it is not the same as paid and briefed.',
    ],
  },
  {
    title: 'ACTI / source post still controls the launch',
    tags: ['watch', 'launch day', 'source link'],
    points: [
      'AK locked June 30 at 7:30 AM PT and sent advance launch copy, but the thread still depends on the real official post link once the announcement is live.',
      'Do not treat draft copy or confidential assets as the final thing Robert should quote.',
      'Keep this in watch until the canonical live tweet lands and someone hands Robert the exact source URL.',
      'Pre-launch copy is prep, not execution clearance.',
    ],
  },
  {
    title: 'Atomik / launch fixed, receipt still owed',
    tags: ['watch', 'receipt promised', 'old-vs-new split'],
    points: [
      'The Clodo launch itself moved, the wrong tag got corrected via comment, and Aviral thanked Asher for the fix.',
      'What is still missing is the actual receipt that was promised before go-live, not more execution work.',
      'Keep the old OpenClaw finance proof separate from any fresh Clodo follow-on conversation.',
      'A payment-cleared claim is still not closed finance until the receipt is in-thread.',
    ],
  },
  {
    title: 'HashMatrix / interest yes, scope no',
    tags: ['watch', 'lead reply', 'underspecified'],
    points: [
      'Stella followed up on the consecutive-post idea, but the thread still has no count, cadence, dates, or format mix to price against.',
      'The accidental question-mark send added noise but did not materially move scope forward.',
      'Watch for a real scope reply before treating this as a quote-ready campaign.',
      'Open interest does not replace concrete scope.',
    ],
  },
  {
    title: 'iMerch / paid but still waiting on go',
    tags: ['watch', 'paid', 'client hold'],
    points: [
      'Peter already sent the receipts and backup creative, but the team still wants to wait for the right Elon rocket post before choosing the live path.',
      'Nothing should publish until Peter sends an explicit go or stand-down message.',
      'Keep this warm, but do not let it outrank launches with fixed dates and missing payment proof.',
...
const V4_COMPANY_OS_RULES = [
  ...
  'Product-test campaigns do not move forward on goodwill alone. If access, credits, or connectors fail, document the failure and push support before Robert time is spent.',
