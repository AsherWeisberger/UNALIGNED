...
function V3InlineReply({ lead, user }) {
  ...
  const missingLeadRecipient = !internalOnly && !to.length;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 7, width: '100%' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 7 }}>
        <input className="brief-input" value={subject} readOnly title="Subject" />
      </div>
      <textarea
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ flex: 1, fontSize: 11.5, color: error ? 'var(--bad)' : 'var(--text-3)' }}>
          {error || (missingLeadRecipient ? `This card does not have an outside lead email for ${V3SenderName(sender)}. Add one above or use Talk internally.` : isSelfRecipient ? `${V3SenderName(sender)} is also a recipient. Remove them before sending.` : status === 'sent' ? 'Sent.' : `${internalOnly ? 'Internal only' : 'Lead chain'} · sending as ${V3SenderName(sender)}${lead.gmailThreadId && sender === 'robert' ? ' in the Gmail thread' : ''}`)}
        </span>
        <button className="btn btn-sm btn-ghost" type="button" disabled={status === 'sending'} onClick={() => setBody(lead.draftReply?.body || '')}>
          <V3Icon name="spark" w={12} /> Draft
        </button>
        <button className="btn btn-sm btn-accent" type="button" disabled={status === 'sending'} onClick={send}>
          <V3Icon name="send" w={12} /> {status === 'sending' ? 'Sending...' : 'Send'}
        </button>
      </div>
    </div>
  );
