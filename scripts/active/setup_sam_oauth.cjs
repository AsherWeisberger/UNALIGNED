...
const EXPECTED_EMAIL = 'unalignedx@gmail.com';
const SCOPES = [
...
async function main() {
  ...
  console.log('');
  console.log('Authorize Sam Gmail OAuth');
  console.log('1. Sign into UnalignedX@gmail.com in the browser that opens.');
  ...
  const code = await new Promise((resolve, reject) => {
    ...
    server.on('request', (req, res) => {
      ...
      clearTimeout(timeout);
      res.writeHead(200, { 'Content-Type': 'text/html' }).end('<h1>Sam Gmail connected.</h1><p>You can close this tab.</p>');
      resolve(authCode);
    });
  ...
  if (!tokens.refresh_token) {
    throw new Error('Google did not return a refresh token. Re-run this script and make sure you approve consent for the Sam account.');
  }

  let email = 'UnalignedX@gmail.com';
  try {
  ...
  if (email.toLowerCase() !== EXPECTED_EMAIL) {
    throw new Error(`Authorized ${email}, but expected UnalignedX@gmail.com. No Firebase secret was written.`);
  }

  await db.collection('_secrets').doc('sam_gmail_oauth').set({
    email,
  ...
  console.log('');
  console.log('Sam Gmail OAuth saved to Firebase: _secrets/sam_gmail_oauth');
}
